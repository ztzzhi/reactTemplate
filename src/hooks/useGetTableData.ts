...
interface IExtraParams {
  ...
  cachePageCallback?: (current: number, size: number, prevSize: number) => void //分页切换缓存
  paramsCallback?: (params: Record<string, any>) => Record<string, any>
}
const useGetTableData = <T>(
  networkRequest: (...set: any) => Promise<IResponseList<T>>,
  ...
  const initParams = {
    current: 1,
    size: extraParams?.pageSize ? extraParams.pageSize : 10,
  }
  ...
  const [tableProps, setTableProps] = useState<ITableProps<T>>({
    ...
    pagination: {
      current: 1,
      pageSize: extraParams?.pageSize ? extraParams.pageSize : 10,
      total: 0,
      onChange: (current: number, size: number) => {
        setParams((prev) => {
          extraParams?.cachePageCallback?.(current, size, prev.size)
          return {
            ...prev,
            current: prev.size === size ? current : 1,
            size,
          }
        })
      },
  ...
  const search = () => {
    // search不改变pageSize
    const paramsData = { ...params, current: 1, ...extraParams?.defaultParams, ...extraParams?.form?.getFieldsValue() }
  ...
  const updateTableProps = (res?: IResponseList<T>) => {
    setTableProps((prev) => ({
      ...prev,
      loading: false,
